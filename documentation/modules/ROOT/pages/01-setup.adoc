...
And then you are ready for installing ArgoCD on Kubernetes.

[#install_argocd]
== ArgoCD Installation

In this section we will be exploring how to install ArgoCD on Minikube and how to install it on OpenShift with the OpenShift GitOps Operator.
...
[NOTE]
======
It will take few mins for the ArgoCD components to be installed, you an watch the status using the command:
...
To install it, you need to follow the instructions written in the: link:https://docs.openshift.com/container-platform/4.7/cicd/gitops/installing-openshift-gitops.html[OpenShift GitOps installation documentation,window='_blank'].

The OpenShift GitOps Operator is installed via the Operator Hub. You
...
image::os-gitops-installed.png[OpenShift GitOps Installed]
