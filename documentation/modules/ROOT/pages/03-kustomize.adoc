...
+
--
.link:https://github.com/redhat-developer-demos/openshift-gitops-examples/tree/minikubr/apps/bgd/overlays/bgdk[bgdk-app.yaml,window='_blank']
[source,yaml,subs="+macros,attributes+"]
----
...
at the `examples/minikube/bgdk-overlay/kustomization.yaml` file.
...
Add Minikube IP (`minikube ip`) and the Ingress hostname `bgdx.devnation` to your Host file, like `/etc/hosts`.

Example:
[source,bash,subs="attributes+,+macros"]
----
192.168.39.242 bgd.devnation bgdx.devnation
----
...
This is using a concept of an "overlay", where you have a "base"
set of manifests and you overlay your customizations. Take a look
at the `examples/bgdk-overlay/kustomization.yaml` example
file.
