...
Taking a look at the `kustomize-build/welcome.yaml`:
...
[source,bash,subs="attributes+,+macros"]
----
oc apply -f resources/bgd-app/bgd-app.yaml
----
...
`oc apply -f resources/bgdk-app/bgdk-app.yaml`{{execute}}
